...
use takobox::{Directories, app, model::settings::Settings};
...
pub fn create_test_app(pool: PgPool) -> TestServer {
    ...
    let tmp_dirs = Directories::new("/tmp/takobox_data".to_string());
    let app = app(session_secret, pool, tmp_dirs, Settings::default());
